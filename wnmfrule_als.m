function [A,Y,numIter,tElapsed,finalResidual]=wnmfrule_als(X,W,k,lam)
...
tStart=tic;
optionDefault.distance='als';
optionDefault.iter=1000; %1000; TEMP CHANGE
...
% CHANGING WEIGHT MATRIX HERE  -- NOW PASSING WEIGHT MATRIX INTO FUNCTION
% Weight
% Orig beg
% W=isnan(X);
% X(W)=0;
...
for i=1:option.iter
    switch option.distance
        case 'als'
            A=A.*(((W.*X)*Y')./((W.*(A*Y))*Y'+(A*lam)));
%             A(A<eps)=0;
                A=max(A,eps);
            Y=Y.*((A'*(W.*X))./(A'*(W.*(A*Y))+(Y*lam)));
%             Y(Y<eps)=0;
                Y=max(Y,eps);
        case 'kl'
            A=(A./(W*Y')) .* ( ((W.*X)./(A*Y))*Y');
    ...
    if mod(i,10)==0 || i==option.iter
        ...
        curRes=norm(W.*(X-XfitThis),'fro')+lam*(norm(Y,'fro')+norm(A,'fro')); 
